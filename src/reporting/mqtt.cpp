...
// TODO(DIMI) - subscribe to reset / configure commands in topic ?
...
void MqttNotifier::NotifyAttackOccurred(Message attackMessage) {
  setAttackState(true);
  mqttClient.publish("security/canary/attackerip", 2, true,
                     attackMessage.attackerIp.c_str());
}

void MqttNotifier::ResetAttackState() { setAttackState(false); }
...
void setMqttHost() {
  ...
  Serial.printf("[INFO]: MQTT Parsed Host: %u.%u.%u.%u \r\n", mqttHost[0],
                mqttHost[1], mqttHost[2], mqttHost[3]);
