...
// TODO(DIMI) - use pointers or char[] for local things
...
void findIpInsideArpRequest() {
  ...
  const String attackerIP =
      s.substring(startIndex + tellString.length() + 1, endIndex - 1);
  ...
  // Each ARP <someone> -> <this> request is followed by <this> -> <someone>
  // request
...
void PingWatcher::setup() {
  netDump.printDump(streamString, Packet::PacketDetail::CHAR,
                    [](Packet n) { return (n.isICMP() || n.isARP()); });
}
